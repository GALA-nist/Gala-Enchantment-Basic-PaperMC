...
public class EffectListener implements Listener {
    ...
    public void onHit(EntityDamageByEntityEvent event) {
        ...
        for (GEBEnchant enchant : plugin.enchants().getOnItem(weapon)) {
            if (enchant.type() == GEBEnchant.Type.EFFECT) {
                ...
                victim.addPotionEffect(new PotionEffect(type, 20 * 5 * enchant.level(),
                        enchant.level() - 1, false, true, true));
            } else {
                applyCustomOnHit(attacker, victim, enchant);
            }
        }
    }

    private void applyCustomOnHit(Player attacker, LivingEntity victim, GEBEnchant enchant) {
        CustomEffect effect;
        try {
        ...
        switch (effect) {
            ...
            case IGNITE ->
                    victim.setFireTicks(Math.max(victim.getFireTicks(), 20 * 3 * enchant.level()));
            default -> { }
        }
    }
    ...
    public void onBreak(BlockBreakEvent event) {
        ...
        for (GEBEnchant enchant : plugin.enchants().getOnItem(tool)) {
            ...
            if (effect == CustomEffect.HARVESTERS_MEAL) {
                ...
                if (ThreadLocalRandom.current().nextDouble() < 0.10 * enchant.level()) {
