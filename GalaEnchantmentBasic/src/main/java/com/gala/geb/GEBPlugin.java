...
import com.gala.geb.listener.FlightListener;
import com.gala.geb.listener.GrindstoneListener;
...
public final class GEBPlugin extends JavaPlugin {
    ...
    private EquipEffectTask equipTask;
    ...
    public void onEnable() {
        ...
        // Fast fixed cycle: Dreamcatcher safety every run, potion effects on
        // the slower effect-refresh-ticks interval inside the task.
        equipTask = new EquipEffectTask(this);
        equipTask.runTaskTimer(this, EquipEffectTask.PERIOD, EquipEffectTask.PERIOD);

        printBanner();
    }

    private void printBanner() {
        ComponentLogger log = getComponentLogger();
        String version = getPluginMeta().getVersion();
        log.info(Component.text("--------------------------------------------------", NamedTextColor.DARK_PURPLE));
        log.info(Component.text("   ________ ", NamedTextColor.LIGHT_PURPLE));
        log.info(Component.text("  |  GALA  |   ", NamedTextColor.LIGHT_PURPLE)
                .append(Component.text("Gala Enchantment Basic v" + version, NamedTextColor.GREEN)));
        log.info(Component.text("  |________|   ", NamedTextColor.LIGHT_PURPLE)
                .append(Component.text("Running on " + getServer().getName()
                        + " - " + getServer().getVersion(), NamedTextColor.GREEN)));
        log.info(Component.text(""));
        log.info(Component.text("  Made by: ", NamedTextColor.GRAY)
                .append(Component.text("Gala", NamedTextColor.AQUA))
                .append(Component.text("  |  Custom enchantments for tools, weapons & armor", NamedTextColor.GRAY)));
        log.info(Component.text("  ")
                .append(Component.text(enchantManager.all().size() + " enchantment(s) loaded. ", NamedTextColor.YELLOW))
                .append(Component.text("Thanks for using GEB!", NamedTextColor.GREEN)));
        log.info(Component.text("--------------------------------------------------", NamedTextColor.DARK_PURPLE));
    }
    ...
    public EquipEffectTask equipTask() { return equipTask; }
}
